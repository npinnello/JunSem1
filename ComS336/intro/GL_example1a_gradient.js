...
void main()
{
	// convert the x framebuffer coordinate into a value in [0, 1]
	// (assumes possible range of x values is 100 to 300)
	float val = (gl_FragCoord.x - 100.0) / 200.0;
  gl_FragColor = vec4(val, 0.0, 0.0, 1.0);

  vec4 red = vec4(1.0, 0.0, 0.0, 1.0);
  vec4 green = vec4(0.0, 1.0, 0.0, 1.0);
  vec4 c = (1.0 - val) * red + val * green;

  // alternatively, use the built-in mix() function
  //vec4 c = mix(red, green, val);
  gl_FragColor = c;
